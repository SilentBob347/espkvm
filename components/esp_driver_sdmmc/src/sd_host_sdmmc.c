...
static esp_err_t sd_host_slot_clock_update_command(sd_host_sdmmc_slot_t *slot, bool is_cmd11)
{
    ...
    esp_err_t err = sd_host_slot_start_command(slot, cmd_val, 0);
    /* espkvm: after a failed write the controller can be left waiting for data
     * that never comes, and it will not take a clock update that waits for it.
     * Every later command then fails the same way. Reset it and send the update
     * without the wait. */
    if (err == ESP_ERR_TIMEOUT && !is_cmd11) {
        ESP_LOGW(TAG, "clock update not taken; resetting the controller");
        s_module_reset(slot->ctlr);
        for (int i = 0; i < 1000 && !s_is_module_reset_done(slot->ctlr); i++) {
            esp_rom_delay_us(100);
        }
        cmd_val.wait_complete = 0;
        err = sd_host_slot_start_command(slot, cmd_val, 0);
    }
    ESP_RETURN_ON_ERROR(err, TAG, "sd_host_start_command returned 0x%x", err);
