...
void sd_host_fill_dma_descriptors(sd_host_sdmmc_slot_t *slot, size_t num_desc)
{
    ...
    /* espkvm: the ring wraps at its size, not at num_desc (the refill count) */
    const size_t ring = slot->ctlr->dma_desc_num;
    for (size_t i = 0; i < num_desc; ++i) {
        ...
        desc->next_desc_ptr = (last) ? NULL : slot->ctlr->dma_desc + ((next + 1) % ring);
        ...
        cur_trans->next_desc = (cur_trans->next_desc + 1) % ring;
...
{
    /* espkvm: reset the IDMAC before every transfer. After an aborted transfer it
     * stays suspended on a stale descriptor and ignores the new DBADDR, so the
     * next transfer waits forever with no interrupt. The resets self-clear. */
    SDMMC.ctrl.dma_reset = 1;
    SDMMC.bmod.sw_reset = 1;
    for (int i = 0; i < 1000 && (SDMMC.ctrl.dma_reset || SDMMC.bmod.sw_reset); i++) {
        esp_rom_delay_us(10);
    }
...
/* espkvm: set when the command itself failed, so no data phase started */
static bool s_cmd_phase_failed;
/* espkvm: set when the data phase failed; the controller is then reset */
static bool s_data_failed;
    ...
    while (next_state != state) {
        ...
        case SDMMC_SENDING_DATA:
            if (mask_check_and_clear(&evt.sdmmc_status, SD_DATA_ERR_MASK)) {
                ...
                sd_host_dma_stop(slot);
                /* espkvm: with DMA stopped the controller waits for data that
                 * never comes; end the transfer here and reset it afterwards */
                s_data_failed = true;
                next_state = SDMMC_IDLE;
                break;
            }
        ...
        case SDMMC_BUSY:
            ...
            process_data_status(slot, orig_evt.sdmmc_status, cmd);
            s_data_failed = cmd->error != ESP_OK;
            next_state = SDMMC_IDLE;
            break;
        }
...
/* espkvm: a multi-block transfer that fails in its data phase never gets the
 * controller's auto-stop, so the card stays in its receive/send state and
 * refuses every later command. Send CMD12 to bring it back to transfer. */
static void send_stop_after_error(sd_host_sdmmc_slot_t *slot)
{
    ...
    esp_err_t err = sd_host_slot_start_command(slot, make_hw_cmd(&stop), 0);
    sdmmc_req_state_t st = SDMMC_SENDING_CMD;
    sd_host_sdmmc_event_t unhandled = {};
    while (err == ESP_OK && st != SDMMC_IDLE) {
        err = handle_event(slot, &stop, &st, &unhandled);
    }
    if (err == ESP_OK && stop.error == ESP_OK && !wait_for_busy_cleared(slot, stop.timeout_ms)) {
        err = ESP_ERR_TIMEOUT;
    }
    if (err != ESP_OK || stop.error != ESP_OK) {
        ESP_LOGW(TAG, "CMD12 after a failed transfer: 0x%x/0x%x", err, stop.error);
    }
}

/* espkvm: bring the controller out of a failed or hung data transfer */
static void reset_after_data_error(sd_host_sdmmc_slot_t *slot)
{
    sdmmc_ll_stop_dma(slot->ctlr->hal.dev);
    ...
    SDMMC.ctrl.dma_reset = 1;
    SDMMC.bmod.sw_reset = 1;
    for (int i = 0; i < 10000 && (SDMMC.ctrl.controller_reset || SDMMC.ctrl.fifo_reset ||
                                  SDMMC.ctrl.dma_reset || SDMMC.bmod.sw_reset); i++) {
        esp_rom_delay_us(10);
    }
    sdmmc_ll_clear_interrupt(slot->ctlr->hal.dev, 0xffffffff);
    sdmmc_ll_clear_idsts_interrupt(slot->ctlr->hal.dev, 0xffffffff);
    xQueueReset(slot->ctlr->event_queue);
    esp_err_t err = sd_host_slot_set_card_clk(slot);
    if (err != ESP_OK) {
        ESP_LOGW(TAG, "clock after a data error reset: 0x%x", err);
    }
}
...
esp_err_t sd_host_slot_sdmmc_do_transaction(sd_host_slot_handle_t slot, sdmmc_command_t *cmdinfo)
{
    ...
    if (cmdinfo->data && (s_data_failed || (ret != ESP_OK && !s_cmd_phase_failed))) {
        reset_after_data_error(slot_ctx);
    }
    if ((ret != ESP_OK || cmdinfo->error != ESP_OK) && !s_cmd_phase_failed && cmd_needs_auto_stop(cmdinfo)) {
        send_stop_after_error(slot_ctx);
    }
