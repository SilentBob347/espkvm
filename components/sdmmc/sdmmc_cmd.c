...
static esp_err_t write_sectors_dma_once(sdmmc_card_t* card, const void* src,
        size_t start_block, size_t block_count, size_t buffer_len)
{
    ...
    if (err != ESP_OK) {
        if (cmd.opcode == MMC_WRITE_BLOCK_MULTIPLE) {
            ...
            /* espkvm: keep the write's error; ACMD22 answering does not make it a success */
            esp_err_t err_acmd22 = sdmmc_send_cmd_num_of_written_blocks(card, &successfully_written_blocks);
            if (err_acmd22 == ESP_OK) {
                ESP_LOGD(TAG, "%s: wrote %zu blocks out of %zu", __func__, successfully_written_blocks, block_count);
            } else {
                ESP_LOGE(TAG, "%s: sdmmc_send_cmd_num_of_written_blocks returned 0x%x", __func__, err_acmd22);
            }
        }
        if (err_cmd13 == ESP_OK) {
...
/* espkvm: a failed transfer leaves the card back in transfer state (the host
 * sends CMD12), so the same sectors can simply be written or read again. */
#define SDMMC_WRITE_ATTEMPTS 5
#define SDMMC_READ_ATTEMPTS 5

/* espkvm: told when a sector read or write starts, so the application knows
 * when the card was last busy. */
__attribute__((weak)) void sdmmc_espkvm_transfer_started(sdmmc_card_t *card)
{
    (void)card;
}

/* espkvm: told about every failed attempt, before the retry. The application
 * overrides it to slow the bus down; the retry then runs at the new clock. */
__attribute__((weak)) void sdmmc_espkvm_transfer_failed(sdmmc_card_t *card, bool write, esp_err_t err)
{
    (void)card;
    (void)write;
    (void)err;
...
esp_err_t sdmmc_write_sectors_dma(sdmmc_card_t* card, const void* src,
        size_t start_block, size_t block_count, size_t buffer_len)
{
    sdmmc_espkvm_transfer_started(card);
    esp_err_t err = ESP_FAIL;
    for (int attempt = 1; attempt <= SDMMC_WRITE_ATTEMPTS; attempt++) {
        err = write_sectors_dma_once(card, src, start_block, block_count, buffer_len);
        if (err == ESP_OK || err == ESP_ERR_INVALID_SIZE || host_is_spi(card)) {
            break;
        }
        ESP_LOGW(TAG, "%s: write of %zu blocks at %zu failed (0x%x), attempt %d/%d",
                 __func__, block_count, start_block, err, attempt, SDMMC_WRITE_ATTEMPTS);
        sdmmc_espkvm_transfer_failed(card, true, err);
    }
    return err;
}
...
static esp_err_t read_sectors_dma_once(sdmmc_card_t* card, void* dst,
        size_t start_block, size_t block_count, size_t buffer_len)
{
    if (start_block + block_count > card->csd.capacity) {
        return ESP_ERR_INVALID_SIZE;
        ...
        size_t start_block, size_t block_count, size_t buffer_len)
{
    sdmmc_espkvm_transfer_started(card);
    esp_err_t err = ESP_FAIL;
    for (int attempt = 1; attempt <= SDMMC_READ_ATTEMPTS; attempt++) {
        err = read_sectors_dma_once(card, dst, start_block, block_count, buffer_len);
        if (err == ESP_OK || err == ESP_ERR_INVALID_SIZE || host_is_spi(card)) {
            break;
        }
        ESP_LOGW(TAG, "%s: read of %zu blocks at %zu failed (0x%x), attempt %d/%d",
                 __func__, block_count, start_block, err, attempt, SDMMC_READ_ATTEMPTS);
        sdmmc_espkvm_transfer_failed(card, false, err);
    }
    return err;
}
